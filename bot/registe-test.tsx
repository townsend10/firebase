...
const schema = z.object({
  ...
  phone: z.string()
    ...
    .regex(/^\(\d{2}\)\s\d{5}-\d{4}$/, { message: 'Formato de telefone inválido. Ex: (XX) XXXXX-XXXX' }),
  password: z.string()
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-6">Cadastrar Usuário</h2>

        {registrationSuccess && (
          <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">
            <strong className="font-bold">Sucesso!</strong>
            <span className="block sm:inline ml-2">Usuário cadastrado com sucesso.</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <InputMask
              mask="(99) 99999-9999"
              maskChar={null} // Não mostra o caractere de máscara restante
              id="phone"
              type="tel"
              {...register('phone')}
              className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
